describe('Register in Smarthop Demo', () => {
    ...
    cy.get('input[name=first_name]').type('Dispatch Service');
    cy.wait(1000);
    cy.get('input[name=last_name]').type('Testing');
    cy.wait(1000);
    cy.get('input[type=email]').type('ndiaz+carrier415@smarthop.co');
    cy.get('input[name=phone]').type('+573112163858');
    cy.wait(2000);
    cy.get('#mui-component-select-typeCompany').click();
    cy.get('li[data-value=DISPATCH_SERVICE]').click();
